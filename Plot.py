...
#pr_train = pd.read_csv("periods_train.csv", parse_dates=["activation_date", "date_from", "date_to"])
...
train_df['weekday_en'] = train_df['wday'].apply(lambda x : daymap[x])
...
interval1=(-0.99, 0.50, 0.80, 1.1)
cats1=['0-0.5','0.5-0.8','0.8-1.0']
interval2=(-0.99, 5, 10, 15, 20, 25, 30)
cats2=['0-5','5-10','10-15','15-20','20-25','25up']
#train_df['deal_class'] = train_df['deal_probability'].apply(lambda x: ">=0.8" if x >=0.8 else "))
train_df["deal_class_2"] = pd.cut(train_df.deal_probability, interval, labels=cats)
train_df["deal_class_1"] = pd.cut(train_df.deal_probability, interval1, labels=cats1)
train_df["price_log_intrl"]=pd.cut(train_df.price_log, interval2, labels=cats2)
...
def generate_histogram_plot(df, col, title):
    ...
    layout = go.Layout(title=title, legend=dict(orientation="h"), height=500)
...
def _create_pie_chart(df, col, title, color, w=None, h=None, lm=0, limit=100):
    ...
    layout = dict(title=title, margin=dict(l=lm), width=w, height=h)
    #layout = go.Layout(title=title, width=600, height=400, margin=dict(l=100))
    data = [trace]
    fig = go.Figure(data=data, layout=layout)
    iplot(fig)
    
def _create_box_plot_bin(df, col1, col2, y="deal_probability", w=None, h=None, lm=0, limit=100):
    sns.set(rc={'figure.figsize':(15, 8)})
    sns.boxplot(x=col1, y=y, hue=col2,  palette="PRGn", data=train_df)
    plt.title("Deal probability by "+ col1 + " and " + str(col2))
...
generate_histogram_plot(train_df,'deal_probability', "Distribution of deal_probability")


# In[65]:


#another kind of deal_prrob. hist.
generate_histogram_plot(train_df,'deal_class_2', "Distribution of grouped deal probability")


# In[ ]:


generate_histogram_plot(train_df,'price_log', "Distribution of log price")
generate_histogram_plot(train_df,'price_log_intrvl', "Distribution of log price")


# In[48]:


_generate_bar_plot_hor(train_df, 'parent_category_name_en', "Distribution of Parent Category", '#f2b5bc', 600, 400, 200)


# In[95]:


_generate_bar_plot_hor(train_df, 'category_name_en', "Distribution of Category", '#f2b5bc', 600, 500, 200)


# In[40]:


_generate_bar_plot_ver(train_df, 'price', "Distribution of price", '#f2b5bc', 700, 400, 200, limit=30)
#_generate_bar_plot_ver(train_df, 'price_log', "Distribution of log price", '#f2b5bc', 700, 400, 200, limit=30)


# In[52]:


trace1 = _generate_bar_plot_ver(train_df, 'param_1', "Param 1 Values", '#1bf7bc', 700, 400, 200, limit=30, need_trace = True)
trace2 = _generate_bar_plot_ver(train_df, 'param_2', "Param 2 Values", '#1bf7bc', 700, 400, 200, limit=30, need_trace = True)
trace3 = _generate_bar_plot_ver(train_df, 'param_3', "Param 3 Values", '#1bf7bc', 700, 400, 200, limit=30, need_trace = True)
...
#_generate_bar_plot_hor(train_df, cols[3], "Distribution of City", '#c2e2a3', 600, 600, 150, limit=30)


# In[87]:


#_generate_bar_plot_ver(df, col, title, color, w=None, h=None, lm=0, limit=100, need_trace = False)
#_generate_bar_plot_ver(train_df, 'month', "Distribution of posted ads in different months ", '#d38bed', 500, 400, 100, limit=30)
print(np.unique(train_df['month']))
_generate_bar_plot_ver(train_df, 'day', "Distribution of posted ads in different month days", '#d38bed', 500, 400, 100, limit=30)
_generate_bar_plot_ver(train_df,'weekday_en' , "Distribution of posted ads in different week days", '#d38bed', 500, 400, 100, limit=30)


# In[85]:


_create_pie_chart(train_df,'user_type' , "Distribution of User Type", '#f2b5bc', 500, 400, 100)
...
##names = train_df["description"][~pd.isnull(train_df["description"])]
#print(names)
...
# deal_prob. box plot
_create_box_plot_bin(train_df, "parent_category_name_en", None)
_create_box_plot_bin(train_df, "parent_category_name_en", "user_type")


# In[32]:


_create_box_plot_bin(train_df, "region_en", None)
_create_box_plot_bin(train_df, "region_en", "user_type")


# In[34]:


_create_box_plot_bin(train_df, "price_log_intrl", None)
_create_box_plot_bin(train_df, "price_log_intrl", "user_type")


# In[35]:


_create_box_plot_bin(train_df, 'title_len',None)
_create_box_plot_bin(train_df, 'title_len',"user_type")


# In[53]:


#_create_box_plot_bin(train_df, 'wday',None)
#_create_box_plot_bin(train_df, 'wday',"user_type")
...
#price log box plot
_create_box_plot_bin(train_df, 'deal_class_1', None,"price_log")
_create_box_plot_bin(train_df, 'deal_class_1',"user_type","price_log")
_create_box_plot_bin(train_df, 'deal_class_1',"parent_category_name_en","price_log")


# In[45]:


_create_box_plot_bin(train_df, 'region_en', None,"price_log")


# In[47]:


_create_box_plot_bin(train_df, 'deal_class_1', "weekday_en", "price_log")
